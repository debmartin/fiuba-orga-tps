...
extern void generic_plot(void*);
extern void sse_plot(void*);
...
pthreads_plot(void)
{
	...
	memset(&vparms, 0, (sizeof(param_t) * nthreads_y));
...
#define SZ(x) sizeof(x)
	...
	if ((tinfo = malloc(SZ(struct thread_info) * nthreads_x)) == NULL) {
		fprintf(stderr, "cannot allocate memory.\n");
		exit(1);
	}
	...
	for (tn = 0; tn < nthreads_x; ++tn) {
		memcpy(&tinfo[tn].vparms, &vparms, SZ(vparms));
		for (i = tn; i < (nthreads_y + tn); i++){
			memcpy(&tinfo[tn].vparms[i - tn], &parms, SZ(parms));
			(tinfo[tn].vparms[i - tn]).y0 = y0 + (i - tn) * yd;
			(tinfo[tn].vparms[i - tn]).y1 = i < (nthreads_y + tn - 1)
								 ? (y0 + (i - tn + 1) * yd)
								 : (y_res);
			(tinfo[tn].vparms[i - tn]).x0 = x0 + (i % nthreads_x) * xd;
			(tinfo[tn].vparms[i - tn]).x1 = (i % nthreads_x) < (nthreads_x - 1)
								 ? (x0 + (i % nthreads_x + 1) * xd)
								 : (x_res);
			}
		void* args[2];
		args[0] = &tinfo[tn].vparms;
		args[1] = &nthreads_y;
